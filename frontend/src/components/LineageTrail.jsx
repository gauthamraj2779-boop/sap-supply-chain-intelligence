import { motion } from 'framer-motion';

export default function LineageTrail({ selectedNode, defaultLineage }) {
  const lineage = selectedNode?.lineage || defaultLineage;
  const nodeTitle = selectedNode
    ? (selectedNode.label || '').replace('\n', ' — ').split('(')[0].trim()
    : 'Apex Microelectronics';

  return (
    <div className="lineage-inspector">
      {/* Left: SAP lineage trail */}
      <div className="li-left">
        <div className="li-title">Selected node — {nodeTitle}</div>
        {!lineage || lineage.length === 0 ? (
          <p className="li-empty">Click any node in the graph to inspect its source record.</p>
        ) : (
          <div className="li-trail">
            {lineage.map((step, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, x: -6 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: i * 0.08 }}
              >
                <div className="li-trail-row">
                  <span className="ltr-table">{step.table}</span>
                  <span className="ltr-field">{step.field}</span>
                  <span className="ltr-value">{step.value}</span>
                  <span className="ltr-desc">{step.description}</span>
                </div>
              </motion.div>
            ))}
          </div>
        )}
      </div>

      {/* Right: prompt to click */}
      <div className="li-right">
        <div className="li-title">Lineage inspector</div>
        {selectedNode ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <p style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.6 }}>
              <strong>{(selectedNode.label || '').replace('\n', ' ')}</strong>
            </p>
            <p style={{ fontSize: 12, color: 'var(--ink-3)', lineHeight: 1.6 }}>
              SAP table: <span style={{ fontFamily: 'var(--mono)', color: 'var(--ink)' }}>{selectedNode.sap_table}</span>
            </p>
            {selectedNode.sap_fields && Object.entries(selectedNode.sap_fields).map(([k, v]) => (
              <div key={k} style={{ display: 'flex', gap: 8, fontSize: 12, fontFamily: 'var(--mono)' }}>
                <span style={{ color: 'var(--ink-4)', minWidth: 56 }}>{k}</span>
                <span style={{ color: 'var(--ink)' }}>{String(v)}</span>
              </div>
            ))}
          </div>
        ) : (
          <p className="li-empty">
            The lineage panel updates in place — this is how the deterministic engine proves its numbers rather than asserting them.
          </p>
        )}
      </div>
    </div>
  );
}
