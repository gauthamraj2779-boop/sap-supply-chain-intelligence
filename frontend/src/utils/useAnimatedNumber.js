...
export function useCountUp(target, options = {}) {
  ...
  useEffect(() => {
    ...
    // Browsers suspend requestAnimationFrame in a hidden tab, so a report
    // rendered in the background would sit at its starting value of 0 -- a
    // wrong number on screen, not merely an unanimated one. Snap to the final
    // value instead, and again if the tab is hidden mid-flight.
...
export function useAnimatedNumber(targetValue, formatFnOrOptions, durationOption = 650) {
  ...
  useEffect(() => {
    ...
    // Same hidden-tab guard as useCountUp: rAF is suspended in a background
    // tab, which would freeze the figure at its previous value.
    if (typeof document !== 'undefined' && document.hidden) {
      setDisplay(formatFn(targetValue));
      prevValue.current = targetValue;
      return;
